...
/-for any propositions x and y, if x is true/you have a proof, then if y is true/you have a proof,
then you can deduce that x and y is true-/
...
/-for all x or y, if x is true then x or y is true-/
def or_intro_right : Prop   := Y → X ∨ Y
def or_elim : Prop          := (X ∨ Y) → (X → Z) → (Y → Z) → Z
/-if proof of x or y, then i have a proof of x implies z-/
